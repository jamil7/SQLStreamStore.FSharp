...
module SqlStreamExtensions =
    ...
    let private subscribeToAll
        (store: IStreamStore)
        (continueAfterPosition: int64)
        (streamMessageReceived: IAllStreamSubscription -> StreamMessage -> CancellationToken -> Async<_>)
        (subscriptionDropped: (IAllStreamSubscription -> SubscriptionDroppedReason -> exn -> unit) option)
        (hasCaughtUp: (bool -> unit) option)
        (prefetchJsonData: bool)
        (name: string)
        ...
        let subscriptionDropped' =
            match subscriptionDropped with
            | Some f -> AllSubscriptionDropped f
            | None -> null

        let hasCaughtUp' =
            match hasCaughtUp with
            | Some f -> HasCaughtUp f
            | None -> null
        ...
        store.SubscribeToAll(
            ...
            prefetchJsonData = prefetchJsonData,
            name = name
        )
    ...
    let private subscribeToStream
        ...
        (subscriptionDropped: (IStreamSubscription -> SubscriptionDroppedReason -> exn -> unit) option)
        (hasCaughtUp: (bool -> unit) option)
        (prefetchJsonData: bool)
        (name: string)
        : IStreamSubscription =
    ...
    type IStreamStore with
        /// Subscribes to the all stream's messages.
        member this.SubscribeToAll
            (
                name: string,
            ...
            let prefetch = defaultArg prefetchJsonData true
        ...
        /// Subscribes to a stream's messages.
